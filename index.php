    ...
    if (isset($_GET['action'])) {
        if ($_GET['action'] === 'connexion') {
            ...
            if (!empty($_POST['login']) AND !empty($_POST['password'])) {
                ...
                throw new Error('Le login et/ou le mot de passe sont incorrects');
            }
        } elseif ($_GET['action'] === 'chapter') {
            if (isset($_GET['id']) && $_GET['id'] > 0) {
                $frontend->chapter($_GET['id']);
            } else {
                throw new Error('Aucun identifiant de chapitre envoyé');
            }
        } elseif ($_GET['action'] =='comment') {
            if (isset($_GET['id']) && $_GET['id'] > 0) {
                $frontend->comment($_GET['id']);
            } else {
                throw new Error('Tous les champs ne sont pas requis!');
            }
            ...
            if (isset($_GET['id']) && $_GET['id'] > 0) {
                ...
                throw new Error('Aucun identifiant de commentaire envoyé');
            }
        } elseif ($_GET['action'] == 'signalComment') {
            if (!empty($_GET['id']) && $_GET['id'] > 0) {
                $frontend->signalComment($_GET['id'], $_GET['chapterId']);
            } else {
                throw new Error('Votre requête n\'a pu aboutir :(');
            }
